  ...
  try {
    ...
    try {
        ...
        if (faucetTokenBalance < tokenAmount) {
             throw new Error(`Faucet has insufficient tokens. Has ${ethers.formatUnits(faucetTokenBalance, decimals)}, needs 0.25`);
        }

        console.log(`Sending 0.25 Tokens (decimals: ${decimals}) to ${address}...`);
        
        // Manual gas limit for token transfer as estimateGas might fail if node is lagging
        const tokenTx = await tokenContract.transfer(address, tokenAmount, { gasLimit: 100000 });
        await tokenTx.wait();
    ...
    return NextResponse.json({ 
        ...
        message: txHashes.length > 1 ? "Sent 0.25 Tokens & 0.001 ETH" : "Sent 0.25 Tokens"
    });
