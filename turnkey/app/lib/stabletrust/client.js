...
export class ConfidentialTransferClient {
  constructor(rpcUrl, contractAddressOrChainId, chainId) {
    ...
    if (!resolvedContractAddress) {
      throw new Error(
        `No Stabletrust contract for chainId ${resolvedChainId}. Supported: 2201, 5042002, 84532, 11155111, 421614, 42431`,
      );
    }
  ...
  async _applyPendingIfNeeded(wallet, privateKey, tokenAddress, actionLabel) {
    ...
    if (pendingBalance.amount > 0n) {
      try {
        ...
        console.warn(
          `Warning: Failed to apply pending balance before ${actionLabel}: ${error.message}`,
        );
      }
    }
  }
  ...
  async confidentialTransfer(
    ...
    try {
      ...
      if (!recipientAccountInfo.exists)
        throw new Error(
          `Recipient account does not exist: ${recipientAddress}`,
        );
      ...
      if (
        ...
        recipientPubkey = Buffer.from(recipientPubkey.slice(2), "hex").toString(
          "base64",
        );
      }
      ...
      if (!currentBalanceCiphertext)
        throw new Error("Current balance ciphertext is required");
      ...
      if (currentBalance === undefined || currentBalance < BigInt(amount))
        throw new Error(
          `Insufficient available balance. Required: ${amount}, Available: ${currentBalance}`,
        );
      ...
      const proof = JSON.parse(
        wasm.generate_transfer_proof(JSON.stringify(proofInput)),
      );
      if (!proof.success)
        throw new Error(
          `Proof generation failed: ${proof.error || "Unknown error"}`,
        );
  ...
  async withdraw(wallet, tokenAddress, amount, options = {}) {
    ...
    try {
      ...
      const wasm = await this._getWasm();
      const proof = JSON.parse(
        wasm.generate_withdraw_proof(JSON.stringify(withdrawInput)),
      );
      if (!proof.success)
        throw new Error(
          `Withdrawal proof generation failed: ${proof.error || "Unknown error"}`,
        );
